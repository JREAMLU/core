...
func handleError(p sarama.AsyncProducer) {
	for {
		...
		if err != nil {
			...
			beego.Error(fmt.Sprintf("producer message error, partition:%d offset:%d key:%v valus:%s error(%v)", err.Msg.Partition, err.Msg.Offset, err.Msg.Key, err.Msg.Value, err.Err))
		}
	}
}
